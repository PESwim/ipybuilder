...
'''
  ...
  * -find update path to IronPython if changed - Must run to update if chenged after first run of ipybuild.
...
def checkhelp(args):
    '''
      filter args:
      -  help  /?,-h, -help
      -  find or update IP location -find 

    '''   
    if any(arg == '-find' for arg in args):
        gsBuild.IPATH = None
        checkRequiredIP()
        print 'Set IP path to {}'.format(gsBuild.IPATH)
        raise SystemExit(0)
    if any(arg in ['/?', '/h', '-h', '-help'] for arg in args):
        if gsBuild.Verbose:
